...
void shuffle(char *a, int n) {
   ...
   /* Die Funktion shuffle ruft die Einträge des Arrays a nach der Länge n hintereinander auf & markiert die Zahl des jeweiligen Indexx in der Variable i, 
	   prüft ob die aufgerufene Zahl i gültig ist, wählt eine zufälle Zahl r, die ebenfalls nach Gültigkeit überprüft wird 
	   und tauscht dann die zufällige Zahl r mit der Zahl an Index i des Arrays. Dies erfolgt solange, bis die Länge des Arrays durchlaufen ist. 
       Die Permutationen des Arrays sind gleich wahrscheinlich, wenn die Funktion uniform gleichverteilt zufällige Zahlen aufruft, da die gleiche Anzahl an Zahlen erstellt und in das Arraya eingefügt werden. */
    for (int i = n - 1; i > 0; i--) {
...
void init_cards(Board *b) {
    require_not_null(b);
	int letters = 0;
	for (int i = 0; i < b->ncards; i++){ // verdeckte Karten
		b->cards[i] = -65 - letters;
		if(i % 2 == 1){
			letters++;
		}
	}
	...
	for (int i = 0; i <= b->rows; i++) {
		for(int j = 0; j <= b->cols; j++) {
			if(i == 0 && j == 0) {
				...
				char printer = b->cards[letters];
				if(printer < 0) printf("# ");
				else printf("%c ", printer);
				letters++;
			}
...
int array_index(Board *b, int r, int c) {
	...
	if (r > b->rows) return 0;
	if (c > b->cols) return 0;
	
	int index_r_c = 0;
	index_r_c = (b->cols * r + c); 
    return index_r_c;
}
...
char get(Board *b, int r, int c) {
	...
	int value_r_c = 0;
	value_r_c = array_index(b, r, c);
	return b->cards[value_r_c];
...
void do_move(Board *b) { /* Die Funktion dreht zwei Karten um und prüft ob die Eingabe des Spielers erlaubt ist.
Das Programm ist robust gegenüber Fehleingaben, da mehrfach geprüft wird ob die Eingabe möglich ist & zum Schluss wird geprüft ob die Karten möglich sind.*/
    require_not_null(b);
    int r1, c1;
